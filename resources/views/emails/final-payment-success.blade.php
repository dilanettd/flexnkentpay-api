...
<div
    ...
    <div style="margin-top: 15px;">
        <h3>{{ __('email.final_payment_seller_info') }}</h3>
        <p><strong>{{ __('email.final_payment_seller_name') }}:</strong> {{ $sellerName }}</p>
        <p><strong>{{ __('email.final_payment_shop_name') }}:</strong> {{ $shop->name }}</p>
        <p><strong>{{ __('email.final_payment_contact') }}:</strong> {{ $sellerPhone }}</p>
        @if($shop->location)
            <p><strong>{{ __('Adresse') }}:</strong> {{ $shop->location }}</p>
        @endif
    </div>
</div>
